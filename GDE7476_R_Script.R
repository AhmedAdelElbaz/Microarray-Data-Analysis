...
#  - Sample_type and unSample_type samples        #
#  - GSE7476 Microarray Analysis         #    
#  - 2022- 12-09                          #
...
celFilesDirectory="D:/MAIN/STUDY/PROGRAMMING/R/MICROARRAY DATA SETS ANALYSIS/GSE7476/GSE7476_RAW"
...
phenotable <- read.csv(paste0(celFilesDirectory,"/phenotable.csv"), header = T)
...
for (sample in sampleNames(affyData)){
  name <- strsplit(sample, ".C")[[1]][1]
  samples <- c(samples , name)
}
rm(name)
sampleNames(affyData) <- samples
phenotable$samples <- samples
#############Exception Here################# 
class(affyData)
...
#phenotable$Sample_type= unlist(sapply(strsplit(phenotable$Sample.Type, "_"),c)[2,])
exp=exp.data.agg
...
groups=unique(phenotable$Sample_type)
group1=groups[1]
group2=groups[2]
group3=groups[3]
group4=groups[4]

group1.columns=  phenotable[ phenotable$Sample_type ==group1,]$samples
group2.columns=  phenotable[ phenotable$Sample_type ==group2,]$samples
group3.columns=  phenotable[ phenotable$Sample_type ==group3,]$samples
group4.columns=  phenotable[ phenotable$Sample_type ==group4,]$samples
...
ggplot(group1.3.genes , aes(x = genes , y = expr , fill = Sample_type)) + geom_bar(stat="identity" , position = 'dodge') + 
  theme(axis.text.x = element_text(angle = 90))
#Volcano Plot
...
### creating  3D PCA
#3D Plot
PCA_res <- as.data.frame(PCA.exp$x)
PCA_res$samples <- row.names(PCA_res)
PCA_res <- merge(PCA_res , phenotable , by = 'samples' , all.x =T)
PCA_res$coloring <- ifelse(PCA_res$Sample_type == group1, 'blue',
                           ifelse(PCA_res$Sample_type == group2, 'green',
                                  ifelse(PCA_res$Sample_type == group3, 'red','black')))

#plot animated 3D plot of PC1 , PC2 , PC3 by plotly
library(plotly)
fig <- plot_ly(PCA_res, x = ~PC1, y = ~PC2, z = ~PC3,
               marker = list(color = ~coloring))
fig <- fig %>% add_markers()
